...
**Catalog V3 Policy:** Никогда не доверять `clean_catalog_v2` без проверки в GS1/НКТ. Запрещены эвристики фото по бренду/весу. Фото — строго на свой CDN R2 (`cdn.korset.app`), 2 ракурса (лицо + оборот), строгий фильтр Gemini Vision. Картинки `korzinavdom` (водяные знаки) — безусловный брак. Детали: `docs/vault/decisions/2026-09-24-catalog-v3-image-and-ean-integrity-policy.md`.
