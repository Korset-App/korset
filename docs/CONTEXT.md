...
- 🔎 **Catalog Search upgrade — Stage 9 RPC v2.1 migration**: создана `supabase/migrations/031_catalog_search_rpc_v2_1.sql` с доводкой до 100%: `normalize_search_quantity()` конвертирует `1л↔1000мл`, `search_category_keywords` lookup table синхронизирован с `categoryMap.js`, token-level `word_similarity` через `unnest(string_to_array(...))` для order-independent matching. Playwright smoke test `tests/e2e/catalogSearch.spec.js`. Changelog: `docs/vault/changelog/2026-05-14-catalog-search-stage9-rpc-v2-1.md`.
