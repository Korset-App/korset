...
Before catalog, product data, scraping, enrichment, images or halal-data work, read
`docs/vault/knowledge/catalog-data-invariants.md`; all its rules remain mandatory.
Never invent EANs, duplicate primary EAN into alternate_eans, overwrite verified data
with inferior data, or use watermarked Korzina v Dom photos. Preserve resumable ingestion.

## Stack — do not substitute

React 18 + Vite · JavaScript, **not** TypeScript · vanilla CSS, **not** Tailwind ·
Supabase (Postgres, Auth, Storage, RLS) · Vercel Serverless · existing AI providers (see task code; no provider substitution)
...
- For project work, read `docs/CONTEXT.md` once per task, then search the relevant area.
- Check `git status --short` before edits; preserve all pre-existing changes.
...
- Run `npm run memory:save` after vault changes (local checks; no remote sync).
- Remote query uses `--remote`; remote save uses `--remote --apply` only with authorization
  for external processing and index writes. Never assume embeddings are free.
- Codex personal memory stores preferences; repository docs are the project source of truth.
...
Default implementation: GPT-6 Sol / medium. Use Luna / low for bounded read-only search,
copy and mechanical edits; Sol / medium for feature work; Astra / high for unresolved
cross-system bugs, security and architecture. These are routing defaults, not permission
to change the active user's chosen model. See `docs/DEVELOPMENT.md` for examples.
When delegating, set the appropriate model explicitly and pass only the task, relevant
paths and constraints, not full chat history. At most two independent helpers; do not
spawn a reviewer for a trivial change or duplicate a completed investigation.
Internal Jev helper: `.agents/skills/korset-workflow/SKILL.md`; use only for worthwhile
batch judgments on Sol/Astra, never automatically for Luna or as a product dependency.

Use `rg --files` to locate files, then scoped `rg -n` and small line ranges. Exclude
session dumps, generated assets and data logs unless needed. Put temporary work in `scratch/`.
Use an `explore` subagent only for a bounded investigation that can run alongside useful
local work; direct search is cheaper for a known file. Delegate no overlapping edits.
Keep one deliverable per task; hand off changed files, checks, blockers and next action.
For existing mixed changes, consult `docs/WORKING_CHANGES.md`; stage explicit paths,
never the whole tree. Assign each parallel edit a single owner and bounded write zone.
Use skills relevant to the task, not every installed skill. Workflow and integration map:
