...
export default function BottomNav() {
  ...
  const getActive = () => {
    ...
    if (pathname.endsWith('/profile') || pathname === '/profile') return 'profile'
    if (pathname.endsWith('/scan') || pathname === '/scan') return 'scan'
    if (pathname.endsWith('/catalog') || pathname === '/catalog' || pathname.includes('/product/'))
      return 'catalog'
    if (pathname.endsWith('/ai') || pathname === '/ai') return 'ai'
    return 'home'
  }

  const active = getActive()
  const col = (on) => (on ? 'var(--nav-active)' : 'var(--nav-muted)')
  ...
  const TABS = [
    {
      ...
      icon: (on) =>
        on ? (
          ...
          <svg width="22" height="22" viewBox="0 0 24 24" fill={col(false)}>
  ...
  return (
    <nav
      style={{
        ...
        zIndex: 100,
        display: 'grid',
        gridTemplateColumns: '1fr 1fr minmax(62px,72px) 1fr 1fr',
        alignItems: 'end',
        columnGap: 4,
      ...
      {TABS.map((tab) => {
        ...
        if (tab.isScan) {
          return (
            <div
              key={tab.id}
              style={{ display: 'flex', justifyContent: 'center', alignItems: 'end', minWidth: 0 }}
            >
              <button
                ...
                style={{
                  ...
                  boxShadow: on
                    ? '0 0 0 6px var(--primary-dim), 0 8px 28px var(--primary-glow)'
                    : '0 0 0 4px var(--primary-dim), 0 4px 20px var(--primary-glow)',
                  flexShrink: 0,
