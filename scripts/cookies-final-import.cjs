...
async function req(method, url, headers = {}, body = null, t = 10000) {
...
async function main() {
  try {
    l('=== COOKIES BAKERY FULL IMPORT (with NPC) ===');
    ...
    // Phase 1: Parent catalog ONLY (225042)
    l('-- Phase 1: Catalog 225042 --');
    ...
    for (let i = 0; i < arr.length; i += 10) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        if (!ean && NPC_KEY && full.name) {
          ...
          try {
            const nr = await req('POST', 'https://nationalcatalog.kz/gw/search/api/v1/search', { 'X-API-KEY': NPC_KEY, 'Content-Type': 'application/json' }, { query: q.substring(0, 80), page: 1, size: 5 }, 5000);
            if (nr.status === 200) {
              const items = JSON.parse(nr.body).items || [];
              const g = items.find(x => x.gtin && /^\d+$/.test(x.gtin.trim()));
              if (g) { const bc = classifyBarcode(g.gtin.trim()); if (bc.valid && bc.ean13) { ean = bc.ean13; stats.npc++; } }
            }
          } catch {}
        }
        ...
        const rec = {
          ...
          ingredients_raw: strip(full.ingredients || full.compound),
        ...
        rec.data_quality_score = Math.min(
          (rec.name ? 15 : 0) + (rec.ingredients_raw ? 25 : 0) + (rec.nutriments_json ? 15 : 0) +
          (rec.image_url ? 15 : 0) + (rec.halal_status === 'yes' ? 10 : 0) + (rec.brand ? 10 : 0) +
          (!ean.startsWith('arbuz_') ? 5 : 0) + (rec.country_of_origin ? 5 : 0), 100);
        processed.push(rec);
      }
    ...
    // Dedup by EAN (keep highest quality)
    ...
    l('\n=== DONE ===');
    l('Found: ' + products.size + ', NPC: ' + stats.npc + ', Created: ' + stats.created + ', Enriched: ' + stats.enriched + ', Errors: ' + stats.errors);
    fs.writeFileSync(path.join(OUT, 'cookies-final-log.txt'), log.join('\n'), 'utf8');
  } catch (e) {
